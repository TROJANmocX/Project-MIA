...
def detect_gestures(show_camera=False):
    mp_hands = mp.solutions.hands
    hands = mp_hands.Hands(min_detection_confidence=0.7, min_tracking_confidence=0.7)
    cap = cv2.VideoCapture(0)
    last_gesture = None
    last_time = 0
    print("Gesture control started. Show gestures to the camera.")
    ...
    while True:
        ...
        if show_camera:
            ...
            # Small sleep to prevent CPU hogging when no window is shown
            time.sleep(0.03)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--show-camera", action="store_true", help="Show camera feed window")
    args = parser.parse_args()
    detect_gestures(show_camera=args.show_camera)
